import styles from './AlignxLogoMotion.module.css';
...
/**
 * 영상 에셋이 없어 채택한 로고 모션 히어로 (00 §3 Q7). stroke-dasharray로 1.2s 드로잉 후
 * 400ms fill 페이드, 뒤로 --soft 색면 3개가 12s 루프로 어긋나며 지나간다. prefers-reduced-motion에서는
 * 전역 리셋(reset.css)이 모든 animation-duration을 거의 0으로 눌러 정적 로고로 자연히 수렴한다.
 */
export function AlignxLogoMotion() {
  return (
    <div className={styles.stage}>
      <div className={`${styles.block} ${styles.block1}`} aria-hidden="true" />
      <div className={`${styles.block} ${styles.block2}`} aria-hidden="true" />
      <div className={`${styles.block} ${styles.block3}`} aria-hidden="true" />
      <svg viewBox="0 0 512 180" className={styles.logo} role="img" aria-label="AlignX">
        {PATHS.map((d) => (
          <path key={d} d={d} pathLength={1} className={styles.shape} />
        ))}
        {POLYGONS.map((points) => (
          <polygon key={points} points={points} pathLength={1} className={styles.shape} />
        ))}
      </svg>
    </div>
  );
