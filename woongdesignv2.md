...
- **다크 모드 채도 보정** — ✅ 웹 구현에서 해결. Soft/Tint/Deep은 라이트 기준 hex를 다크에서 `color-mix`로 배경색 쪽에 섞어 눈부심을 줄인다(`src/styles/tokens.css`). 감사 중 반대 방향 문제 2건도 함께 발견해 고쳤다 — 자세한 대비 수치는 `scripts/contrast-check.mjs` 참고:
  - **Mid/Primary를 텍스트로 직접 쓰면 다크에서 2~4:1로 묻힌다**(반대로 Primary는 Mint/Warm에서 라이트도 미달). Mid는 원래 "라이트 배경 위 텍스트" 전용으로 만든 값이라 다크에 그대로 못 쓴다. → 텍스트 전용 `--accent-text` 토큰 추가: 라이트=Mid, 다크=Soft **원본**(다크 보정 전 hex). Mid/Primary 자체는 배경·보더·차트 용도로는 그대로 둔다.
  - **Mute는 원래 다크 배경 전용으로 튜닝된 값이라 라이트에서 텍스트·반전 배경 모두 1~3:1로 무너진다.** → 라이트에서만 Ink 쪽으로 50% 섞어 어둡힌다, 다크는 원본 유지.
